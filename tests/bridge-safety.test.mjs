...
test("native drag path is input-transparent and exposes press/release IPC", () => {
    ...
    assert.doesNotMatch(dragController, /pendingDrop|settling|committed|cursorRevision|maxSettleMs/);
    ...
    assert.match(dragController, /root\.cancel\("cursor-timeout"\)/);
    assert.match(dragController, /root\.cancel\("stale-release"\)/);
    ...
    assert.match(dragController, /root\.cancel\("lifetime-timeout"\)/);
    ...
    const cursorHandlerEnd = dragController.indexOf("function end");
    ...
    assert.match(manager, /target: GlobalStates[\s\S]*?onSuperDownChanged\(\)[\s\S]*?!GlobalStates\.superDown && dragController\.active[\s\S]*?dragController\.cancel\("modifier-released"\)/);
    assert.doesNotMatch(manager, /name: "snapZonesDragEnd"/);
    assert.match(manager, /name: "snapZonesShiftCancel"[\s\S]*?onReleased:[\s\S]*?dragController\.cancel\("modifier-released"\)/);
...
test("Lua integration preserves native movement and has coherent drop/cancel releases", () => {
    ...
    assert.match(luaIntegration, /quickshell:snapZonesShiftCancel/);
    assert.match(luaIntegration, /qs -c ii ipc call snapZones dragEnd/);
    assert.match(luaIntegration, /release\s*=\s*true/);
    assert.match(luaIntegration, /transparent\s*=\s*true/);
    ...
    assert.match(luaIntegration, /Releasing the mouse commits/);
});
